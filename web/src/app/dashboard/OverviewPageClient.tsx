...
export default function OverviewPageClient() {
  ...
  const { pendingDevices, isLoading: pendingLoading } =
    usePendingDevices(isAdmin);
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <KpiCards
        ...
        totalMediaSize={mediaList.reduce(
          (acc, m) => acc + Number(m.fileSize),
          0,
        )}
          ...
          <Card className="bg-card border-border/60 shadow-xs h-full flex flex-col select-none">
            <CardHeader className="flex flex-row items-center justify-between border-b border-border/40 pb-4 shrink-0">
              <div>
                ...
                <CardDescription className="text-xs text-muted-foreground mt-0.5">
                  Hoạt động realtime từ các màn hình
                </CardDescription>
              </div>
            ...
            <CardContent className="flex-1 overflow-y-auto max-h-[600px] p-4 pr-3 space-y-4 scrollbar-thin">
              {eventLogs.slice(0, 10).map((log) => (
                <div
                  key={log.id}
                  className="flex items-start justify-between border-b border-border/40 pb-3 last:border-b-0 last:pb-0 gap-3">
                  ...
                  <Badge
                    className={`shrink-0 text-[9px] font-semibold tracking-wide border-none px-2 py-0.5 rounded-full ${
                      log.status === "Playback Success"
