...
export default function PlaylistEditor({
    ...
    const loadPlaylistData = async () => {
      if (editingPlaylist) {
        ...
        try {
          ...
          const items = await api.get(`/api/playlists/${editingPlaylist.id}/items`) as BackendPlaylistItem[];
          ...
          if (items && items.length > 0) {
            const mappedSlides: PlaylistItemData[] = items.map(item => ({
              ...
              targetDeviceIds: syncLayout?.deviceMapping?.[item.sortOrder.toString()] || []
            }));
  ...
  }, [playlistName, playlistDesc, selectedResValue, isSyncGroup, slides, draftStatus, editingPlaylist]);
  ...
  const handleUpdateSlideTargetDevices = (deviceIds: string[]) => {
    if (!activeSlideId) return;
    setSlides(prev => prev.map(s => s.id === activeSlideId ? { ...s, targetDeviceIds: deviceIds } : s));
  };
  ...
  const handleSavePlaylist = async () => {
    ...
    try {
      ...
      if (editingPlaylist?.id) {
        ...
        const created = await api.post('/api/playlists', playlistPayload) as CreatedPlaylist;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-foreground">
              {editingPlaylist ? 'Chỉnh sửa Playlist (PPTX)' : 'Thiết kế Playlist Mới (PPTX)'}
            </h3>
            <p className="text-xs text-muted-foreground">Thiết kế danh sách phát quảng cáo dạng các slide chạy tuần tự</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button onClick={handleSavePlaylist} disabled={isSaving} className="bg-primary text-primary-foreground font-semibold">
            {isSaving ? (
              <>
      ...
      {draftStatus === 'detected' && (
        <div className="bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-500 text-xs p-3 rounded-lg flex items-center justify-between gap-4 font-medium animate-in fade-in duration-200">
          <div className="flex items-center gap-2">
            ...
            <span>Phát hiện bản nháp chưa lưu của Playlist. Bạn có muốn khôi phục không?</span>
          </div>
